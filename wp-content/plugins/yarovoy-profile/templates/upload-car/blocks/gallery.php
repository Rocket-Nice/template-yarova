...
$block = yar_get_part_arg( $args, 'block' );
...
<div class="upload-car__gallery">
	<div class="upload-car__gallery-input">
		<input type="file" name="<?= $block['name']; ?>" accept=".jpg, .jpeg" data-validate='{ "ext": [ ".jpg", ".jpeg" ] }'>
		<label for="file">
			<span>Загрузить фотографии автомобиля</span>
	...
	<div class="upload-car__gallery-list <?= ( ! empty( $data['gallery'] ) ? '_active' : '' ) ?>">
		<?php if ( ! empty( $data['gallery'] ) ){
			foreach ( $data['gallery'] as $key => $datum ) { ?>
				<div
					class="upload-car__gallery-item"
					 data-id="<?= $key; ?>"
					 data-image-id="<?= $datum['id']; ?>"
					style="background-image: url(<?= $datum['url'] ?>)"
				>
					<button class="upload-car__gallery-remove profile__icon-remove"></button>
				</div>
			<?php }
		} ?>
	</div>
